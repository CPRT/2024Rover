...
hardware_interface::return_type RoverArmHardwareInterface::read(
  ...
  if (rclcpp::spin_until_future_complete(node, result) ==
    ...
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"),
                "Hardware reading %f %f %f %f for elbow %f %f",
                resultCopy->base, resultCopy->diff1, resultCopy->diff2,
                resultCopy->elbow, resultCopy->wristtilt,
                resultCopy->wristturn);
...
hardware_interface::return_type RoverArmHardwareInterface::write(
    const rclcpp::Time& /*time*/, const rclcpp::Duration& /*period*/) {
  RCLCPP_INFO(rclcpp::get_logger("rclcpp"),
              "Hardware writing %f %f %f %f for elbow %f %f", hw_commands_[0],
              hw_commands_[1], hw_commands_[2], hw_commands_[3],
