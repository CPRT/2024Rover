...
TypingNode::TypingNode() : Node("Typing_controller") {
  subscription_ = this->create_subscription<std_msgs::msg::String>(
      "typing_topic", 10,
      std::bind(&TypingNode::topic_callback, this, std::placeholders::_1));
  subscription2_ = this->create_subscription<
      moveit_msgs::action::ExecuteTrajectory_FeedbackMessage>(
      "execute_trajectory/_action/feedback", 10,
      std::bind(&TypingNode::arm_callback, this, std::placeholders::_1));
...
void TypingNode::getCmd(char k) {
  ...
  if (rclcpp::spin_until_future_complete(node, result) ==
    ...
    RCLCPP_INFO(this->get_logger(), "Letter %lu, Centimeter: %f, %f, %f",
                currLetter, resultCopy->x, resultCopy->y, resultCopy->z);
    cmd.x = resultCopy->x;
    cmd.y = resultCopy->y;
    cmd.z = resultCopy->z;
  }
...
void TypingNode::arm_callback(
    const moveit_msgs::action::ExecuteTrajectory_FeedbackMessage &msg) {
  RCLCPP_INFO(this->get_logger(), "I got %s",
              msg.feedback.state.c_str());  //"IDLE" for when it's done
